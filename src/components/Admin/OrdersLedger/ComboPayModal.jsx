...
export default function ComboPayModal({ receipt, onClose, onPaid }) {
    ...
    //
    // IMPORTANT: this component unmounts itself (`if (!receipt) return null`)
    // as soon as the parent clears the selected receipt in response to
    // onClose(). That used to happen ~synchronously with this call, before
    // the 150ms timer below ever fired — so window.print() ran against an
    // already-unmounted <PrintReceipt>, producing blank pages. printTarget
    // is now rendered outside the `receipt`-gated part of the tree (see the
    // return statement) so it survives the modal closing underneath it.
    ...
    if (!receipt && !printTarget) return null;
